"""Adapted from: https://github.com/deepseek-ai/FlashMLA/blob/main/benchmark/bench_flash_mla.py"""
...
def _mla_attn_kernel(
    ...
    offs_o = cur_batch * stride_o_b + cur_head * stride_o_h + split_kv_id * stride_o_s + offs_d_ckv[None, :]
    tl.store(O + offs_o, acc / e_sum[:, None])
    offs_o_l = cur_batch * stride_o_b + cur_head * stride_o_h + split_kv_id * stride_o_s + HEAD_DIM_CKV
    tl.store(O + offs_o_l, e_max + tl.log(e_sum))
...
@triton.jit
def _mla_softmax_reducekv_kernel(
    Logits,
    B_seq_len,
    O,
    stride_l_b,
...
def _mla_softmax_reducev(
    ...
    _mla_softmax_reducekv_kernel[grid](
        logits,
        b_seq_len,
        o,
        logits.stride(0),
...
def compare_ab(baseline, target, b, s_q, cache_seqlens, h_q, h_kv, d, dv, causal, dtype):
    ...
    block_size = 64
    block_table = torch.arange(b * max_seqlen // block_size, dtype=torch.int32).view(b, max_seqlen // block_size)
